...
<template>
  <b-container fluid="lg">
    <h1>@ Task Manager</h1>
    <b-button v-b-modal.modal-add class="float-right mb-5">Add new task</b-button>
    <b-table :items="items" :fields="fields"> 
      ...
      <template #cell(delete)="data">
          <a v-b-modal.modal-delete @click="handleClickDelete(data.item)" >Delete</a>
      </template>
      <template #cell(detail)="data">
        <a v-b-modal.modal-detail @click="handleClickDetail(data.item)" >Detail</a>
      </template>
    </b-table>
...
export default {
  ...
  methods: {
    ...
    handleDownload(data){
      window.open(`${BACKEND_URL}/download/${this.detailIdDisplay}/${data.name}`)
    }
  }
};
</script>
